...
describe("R04 — durable server-sourced canonical ids", () => {
  ...
  it("MALFORMED SERVER IDS DEGRADE HONESTLY: a garbage metadata.canonicalItemId mints locally (never a thrown read)", async () => {
    const { server, library } = setup();
    server.scriptProfileLibraryRead({
      ok: true,
      value: [
        {
          connectorId: "test-source",
          externalRef: "ref-drifted",
          title: "Drifted Metadata",
          metadata: { canonicalItemId: "not-a-canonical-id" },
        },
      ],
    });
    // The read SUCCEEDS — the malformed id degrades to local minting (the
    // honest fallback), never a thrown library model.
    const model = await library.read();
    expect(model.watchlist.status.state).toBe("ready");
    expect(model.watchlist.entries.length).toBe(1);
    // The entry renders under a LOCALLY-MINTED canonical id (the registry's
    // register path — the pre-R04 behavior).
    expect(model.watchlist.entries[0]?.itemId.startsWith("wfxitm_")).toBe(true);
    expect(model.watchlist.entries[0]?.title).toBe("Drifted Metadata");
  });

  it("MALFORMED SERVER IDS DEGRADE HONESTLY: a garbage history itemId renders without adoption (never a thrown read)", async () => {
    const { server, library } = setup();
    server.scriptHistoryRead({
      ok: true,
      value: [
        {
          // Malformed (non-wfxitm_) — the transport guard only checks
          // non-emptiness; the runtime must not throw on it.
          itemId: "garbage-history-id",
          positionMs: 1_000,
          completed: false,
          lastEventType: "progress",
          updatedAt: new Date(T0).toISOString(),
    ...
    expect(model.history.entries.length).toBe(1);
    expect(model.history.entries[0]?.itemId).toBe("garbage-history-id");
  });
});
