  ...
  async read(query?: LibraryQuery): Promise<LibraryModel> {
    ...
    if (includeWatchlist) {
      for (const entry of serverLibrary) {
        ...
        // R04: when the server row carries a WELL-FORMED canonical item id
        // in metadata.canonicalItemId, the registry ADOPTS it (durable,
        // cross-session) — the locally-minted id (if any) reconciles to
        // the server id. When absent or malformed (server data is never
        // trusted past the transport guard), the runtime mints as before —
        // the honest fallback, never a thrown read.
        const metadata = entry.metadata as Record<string, unknown> | undefined;
        const canonicalItemId = metadata?.canonicalItemId;
        let item: { id: string };
        if (
          typeof canonicalItemId === "string" &&
          isEntertainmentItemId(canonicalItemId)
        ) {
    ...
    for (const entry of serverHistory) {
      ...
      // — only WELL-FORMED canonical ids adopt (the transport guard checks
      // non-emptiness, not the wfxitm_ shape; a malformed id renders in
      // history as-is and never throws the read). The title is honestly
      // the itemId when no source has registered it (the registry's TITLE
      // LAW — never overwrite a real title with the placeholder).
      if (isEntertainmentItemId(entry.itemId)) {
