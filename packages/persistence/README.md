...
**The conversion law (migration 0009, deterministic + idempotent + no
data loss).** Existing realization-keyed rows migrate to canonical keys
in four steps: (1) `ADD COLUMN item_id` (NULL for legacy rows whose
realization has vanished from `source_realizations` — they stay listed
honestly); (2) POPULATE `item_id` by joining `(connector_id,
external_ref)` against `source_realizations` (only fills NULLs —
re-play-safe); (3) MERGE duplicate `(effective_profile, item_id)` rows —
the EARLIEST-SAVED row wins (added_at ASC, external_ref ASC tiebreaker;
its title/metadata survive) and every loser row's realization reference
MERGES into the winner's `metadata.realizations` (addedAt stamped in UTC
with a fixed format — deterministic, never session-timezone dependent;
deduped against what the winner already carries — statement-re-play
safe), then the losers are DELETEd; (4) the canonical-keyed unique index
`library_entries_profile_canonical_key` lands ALONGSIDE the realization-
keyed one (kept).
...
**Cross-source realization replacement on READ (the spec §1 law,
`listAllForProfile`).** Each row answers its EFFECTIVE realization — the
primary reference while it is still LIVE (its `source_realizations` row
exists and keys to the row's canonical item); otherwise the
MOST-RECENTLY-SAVED live realization from the realization set
(`metadata.realizations` — every realization the user ever saved, durably
recorded); otherwise the primary (honest unavailable-for-playback — the
resolve path answers no realizations for a dead ref, and the row stays
listed). The read never MUTATES the row — the effective realization is a
pure projection over the live `source_realizations` state, so a saved
item NEVER breaks when a source disappears IF another realization exists.
