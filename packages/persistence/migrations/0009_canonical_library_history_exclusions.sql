...
--    row wins (added_at ASC, external_ref ASC tiebreaker); the realization
--    sets in metadata.realizations merge into the winner; the title from the
--    earliest row wins. Pure SQL DELETE — the rows survive as the winner.
-- 4. DROP the realization-keyed unique index `library_entries_profile_key`
--    (on (effective_profile, connector_id, external_ref)); CREATE the
--    canonical-keyed unique index `library_entries_profile_canonical_key`
--    on (effective_profile, item_id) WHERE item_id IS NOT NULL. Rows with
--    NULL item_id (realization vanished) are outside the uniqueness domain —
--    they stay listed honestly but cannot be re-saved.
...
-- 3. Merge duplicate (effective_profile, item_id) rows: earliest added_at
--    wins; the realization sets merge in metadata. The DELETE keeps the
--    row with the smallest (added_at, external_ref) per (pkey, item_id)
--    and removes the rest. (Idempotent: a no-op once duplicates are gone.)
