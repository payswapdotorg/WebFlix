...
- **ALSO CONFIRMED (2026-09-15, lead verification after WFX-051/054 merges):** pushes to `main` auto-deploy to **production** — merge `9468977` (WFX-051) produced `dpl_57C6HfBbAQKNZiYvMv4mZTrz7C26` (READY, verified via the API and the production alias health probe) and merge `a906086` (WFX-054) produced `dpl_CifvLUgUScyfnUKwyDu6A6ZoLqNw` (READY). Push-to-main production auto-deploy is therefore OBSERVED, not merely expected. PR comment/status behavior (`gitComments.onPullRequest: true`) remains configured but untested (no PRs opened by this task).
