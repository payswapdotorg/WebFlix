 ...
 * the 052 `watch_history` projection; no-op (mark delivered) for the
 * event types that have no projection consumer yet.
 */
export function makeWatchHistoryDeliverer(db: DbClient): OutboxDeliverer {
      ...
      await db.query(
        `INSERT INTO watch_history (user_id, item_id, position_ms, completed, last_event_type,
         ...
         ON CONFLICT (user_id, item_id) DO UPDATE SET
           position_ms = CASE WHEN $3 > 0
                              THEN GREATEST($3, watch_history.position_ms)
        ...
        [event.userId, event.itemId, positionMs, completed, event.type, event.occurredAt],
      );
    } catch (thrown) {
 ...
 */
export async function runRelayDrain(db: DbClient, clock: Clock, limit = 50): Promise<RelayRunResult> {
  const requeued = await requeueStaleInFlight(db, {
    now: clock.now(),
    olderThanMs: STALE_IN_FLIGHT_MS,
  });
  const drain = await drainEventOutbox(db, {
    deliver: makeWatchHistoryDeliverer(db),
    now: clock.now(),
    limit,
  });
 ...
 */
export function scheduleOpportunisticDrain(db: DbClient, clock: Clock): void {
  eventsSinceDrain += 1;
  const now = clock.now();
  ...
  void runRelayDrain(db, clock, OPPORTUNISTIC_LIMIT)
    .then((result) => {
