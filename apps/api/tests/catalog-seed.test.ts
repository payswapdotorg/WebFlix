...
describe("catalog seed — convergence + idempotency (boot-if-empty, never overwrite)", () => {
  it("the schema baseline is untouched: the migration set re-verifies clean (eleven)", async () => {
    ...
    // migration 0011 (the model and AI controls tables: model policies,
    // envelope-sealed BYOM bindings, transform operations + append-only
    // events — SCHEMA again). The re-verify law is unchanged: nothing new
    // applies, no drift.
