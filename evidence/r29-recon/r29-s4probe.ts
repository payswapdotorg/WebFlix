...
// an INACTIVE labeled item — the corpus 14/400 (the 500 is the active-pill only)
...
// the DRAWER form (<1280, the guide toggle's overlay) — the same corpus measure
await ab("set", "viewport", 1024, 800);
await open(`${BASE}/`);
await sleep(1100);
await evalJS(`document.querySelector('[data-wfx-guide-toggle]')?.setAttribute('data-r29c-guide', '1'); 'ok'`);
