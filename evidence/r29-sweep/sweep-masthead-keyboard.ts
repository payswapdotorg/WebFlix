#!/usr/bin/env bun
// R29 PRODUCTION SWEEP — PROBE 3: MASTHEAD + KEYBOARD MODES. The sign-in pill
// (N12), the gear multi-page menu + the live theme seam (N24/O6-meta), the
// theater re-anchor 1296×729 @(72,68) (N25-b), the per-key keyboard set with
// the /api/playback command path instrumented (N25-a), and the in-app
// miniplayer dock flow (N25-c) — against PRODUCTION.
// Usage: bun evidence/r29-sweep/sweep-masthead-keyboard.ts <base>

import { $ } from "bun";
import { resolve } from "node:path";

const BASE = process.argv[2] ?? "https://webflix-steel.vercel.app";
const OUT = resolve("evidence/r29-sweep/probes/masthead-keyboard.json");
...
const gearOpened = await evalJS(`(() => {
  ...
  if (!g) return false;
  g.setAttribute('data-r29s-gear', '1'); return true;
})()`);
...
if (gearOpened) {
  ...
  // THE SEAM: Light then Dark — data-theme + wfx-theme + theme-color meta together
  await evalJS(`document.querySelector('[data-wfx-appearance="light"]')?.setAttribute('data-r29s-light', '1'); 'ok'`);
  await ab("click", "[data-r29s-light='1']");
  await sleep(1000);
  ...
  await evalJS(`document.querySelector('[data-r29s-gear]')?.click(); 'ok'`);
  await sleep(1000);
  await evalJS(`document.querySelector('[data-wfx-gear-item="shortcuts"]')?.setAttribute('data-r29s-sc', '1'); 'ok'`);
  await ab("click", "[data-r29s-sc='1']");
  await sleep(900);
...
// the served-HTML seam (independent of the browser session)
...
report.checks.metaBoot.servedHtml = {
  ...
  metaContent: rawHtml.match(/<meta[^>]+name="theme-color"[^>]*>/i)?.[0]?.slice(0, 120) ?? null,
...
const watchUrl = hrefs && Array.isArray(hrefs) && hrefs[0] ? (String(hrefs[0]).startsWith("http") ? String(hrefs[0]) : BASE + String(hrefs[0])) : `${BASE}/player`;
const otherUrl = hrefs && Array.isArray(hrefs) && hrefs[1] ? (String(hrefs[1]).startsWith("http") ? String(hrefs[1]) : BASE + String(hrefs[1])) : null;
report.checks.playerUrls = { watchUrl, otherUrl };

// ── N25-b — the theater re-anchor (1296×729 @(72,68) @1440) ──────────────
await open(watchUrl);
await sleep(3000);
const theaterBox = () => evalJS(`(() => {
...
const keySnap = () => evalJS(`(() => ({
  ...
  fullscreenEl: document.fullscreenElement ? (document.fullscreenElement.className || '').toString().slice(0, 40) : null,
...
for (const key of ["m", "ArrowUp", "ArrowDown", "f", "?", "k", " ", "j", "l", "ArrowLeft", "ArrowRight", "5"]) {
  await clearFetches();
  await ab("press", key);
  await sleep(1200);
  report.checks.keyboard[`press_${key.replace(" ", "SPACE")}`] = await keySnap();
  if (key === "m") { await ab("press", "m"); await sleep(700); }
  if (key === "f") { await ab("press", "Escape"); await sleep(800); }
  if (key === "?") { await ab("press", "?"); await sleep(700); }
}
...
// ── N25-c — THE MINIPLAYER FLOW (dock → persist → seam → expand → replace → close) ──
...
// persistence across navigation: / → /search
await open(`${BASE}/`);
await sleep(2000);
...
// the ~3s position-write seam: override the stored position, then watch the rewrite
...
await open(`${BASE}/`);
await sleep(1800);
await evalJS(`document.querySelector('[data-wfx-miniplayer-expand]')?.setAttribute('data-r29s-exp', '1'); 'ok'`);
await ab("click", "[data-r29s-exp='1']");
await sleep(3200);
...
await ab("press", "i");
await sleep(2400);
await evalJS(`document.querySelector('[data-wfx-miniplayer-close]')?.setAttribute('data-r29s-close', '1'); 'ok'`);
...
// leave the session clean (no residue for the captures round)
await evalJS(`sessionStorage.removeItem('wfx-miniplayer'); localStorage.removeItem('wfx-theme'); 'clean'`);

await save();
console.log(`MASTHEAD+KEYBOARD PROBE -> ${OUT}`);
console.log(JSON.stringify(report.checks, null, 1).slice(0, 5600));
