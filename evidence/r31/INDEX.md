...
| §G1 THE SELECTED STATE (the bare checkmark — adjudicated) | §G1 (ERRATUM 2026-09-26: "The SELECTED row ... indicated by a **bare checkmark icon** to its left (never a radio dot). The original 'checkmark inside a box' read was a capture-time over-interpretation — the captured yt-icon carries a single check path, no box outline; adjudicated by the captured DOM + two zoomed re-reads") | The selected row's left slot paints the bare check; the slot is RESERVED on every row (the captured labels all start at x=1164) | `Icon.tsx` (the repo's own existing `check` glyph — the lane-new checkBox icon REMOVED per the adjudication, no dead code) + `AppearancePickerPanel` (the `.wfx-picker__mark` slot, painted only on the selected row; `data-wfx-theme-selected` + `aria-checked` on the menuitemradio role) | the selected-state describes (exactly one marker, the check `<path>` PRESENT, `<rect>` ABSENT, `<circle>` NEVER); `captures/02` (device) + `03` (dark) |
...
- **SHIPPED (bound, cited)**: §G1 in full (the header + back-arrow icon, the
  ...
  geometry with the 56px label indent, the bare-checkmark selected state
  bound to the stored truth — the adjudicated capture grammar, erratum
  2026-09-26 — the two-layer truth, the row-activation path, the
...
- **DIVERGED (honest rows — DIVERGENCES.md)**: §G1's marker — RESOLVED
  2026-09-26 (the lead's three-read adjudication: the capture's bare
  checkmark rules; the code now matches it — ledger row 1), §G2's views/age
